...
| `/authorize` | The page `@elapse/react` opens in a window for every signature: Privy sign-in, the merchant's cap, Face ID permit, Add funds. A window every time, never a frame ([ADR 2026-09-20](../docs/decisions/2026-09-20-authorise-in-a-window-only.md)); the result is posted back to the origin of the session's `success_url`. | Client | `docs/specs/checkout-frd.md` (FR-CHK-038) |
...
The subscriber's meter now lives in the merchant's page, not here: [`@elapse/react`](../sdk/react) renders it and opens `/authorize` in a window for the signatures.
...
- **Mock.** `NEXT_PUBLIC_DASHBOARD_MOCK=1` runs the dashboard against the in-memory mock with a seeded merchant (`demo@elapse.finance`; the sign-in page shows the link instead of emailing it). `/authorize` serves the seeded sessions (`cs_demo`, `cs_ready`, `cs_short`) from that mock without a chain.

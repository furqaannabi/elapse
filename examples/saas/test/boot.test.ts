...
async function run(existing?: Array<{ id: string; name: string; rate_usd_per_second: string; allow_pause?: boolean }>) {
...
describe("FR-EXM-003 npm start", () => {
  it("creates the Product with the §4.2 body, creates a session, prints the three lines", async () => {
    ...
    // FR-EXM-003 amended 2026-09-21: the Product must allow pausing, or FR-EXM-033's Pause never renders.
    expect(api.requests[1]?.body).toEqual({ name: "GPU · 4090", rate_usd_per_second: "0.004", allow_pause: true });
  ...
  it("reuses an existing Product by name instead of creating another", async () => {
    const { api, out } = await run([{ id: "prod_old", name: "GPU · 4090", rate_usd_per_second: "0.004", allow_pause: true }]);
...
describe("FR-EXM-003 (amended) the reused Product must allow pausing", () => {
  it("leaves a Product created before the amendment alone and creates one beside it", async () => {
    const { api, out } = await run([{ id: "prod_old", name: "GPU · 4090", rate_usd_per_second: "0.004", allow_pause: false }]);
    expect(api.requests.map((r) => r.method)).toEqual(["GET", "POST", "POST"]);
    expect(api.requests[1]?.body).toMatchObject({ allow_pause: true });
    expect(out[0]).toBe("Product:  prod_new1  GPU · 4090  $0.004/s");
  });
});

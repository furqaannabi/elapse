...
export function parseRate(rate: string): bigint {
  ...
  // Group 1 is `(\d+)` with no `?`, so a successful match always fills it; the assertion only tells
  // stricter tsconfigs (sdk/react) what the pattern already guarantees. No behaviour change.

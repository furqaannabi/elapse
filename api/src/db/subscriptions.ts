...
export function serializeSubscription(row: SubscriptionRow, now = Math.floor(Date.now() / 1000)) {
  ...
  return {
    ...
    // Always a string. FR-API-140(b) (ADR 2026-09-17): the hosted meter page is retired, so it points to /account.
    ...
    // FR-API-144: seconds already spent in pauses that have **ended**. The meter ticks client-side

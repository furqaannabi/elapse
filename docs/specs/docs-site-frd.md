...
| Id | Requirement | Acceptance |
| --- | --- | --- |
| FR-DOC-010 | Quickstart has exactly these steps: 1 get a test secret key from the dashboard; 2 `npm install @elapse/sdk` and construct `new Elapse({ secretKey, baseUrl })` with the hosted API URL; 3 `products.create`; 4 `checkout.sessions.create` on the server and pass `session.id` to the page; 5 handle `subscription.canceled` with `constructEvent`; 6 run `npx @elapse/cli listen --forward localhost:3000/webhooks`; 7 render `<Authorize>` and `<Meter>` with `@elapse/react`, then authorise on a phone and watch `seconds_elapsed` arrive. Seven steps, not eight: the last one both shows the components and walks the subscriber through, because FR-DOC-047 replaces the hosted URL rather than adding a step. **Amended 2026-09-17 (signed, FR-DOC-047); the step list restated 2026-09-19 to say what replaced the URL.** | Page structure snapshot; each step has a code block. |
...
| FR-DOC-022 | Subscriptions page documents the state machine `incomplete → active → paused \| canceled`, start, pause/resume, cancel, and the elapsed-billing formula (whole seconds × rate) with a worked 83 s example. A "Let subscribers manage their meter" section says pause, resume and stop are the subscriber's actions on the hosted page, that the merchant links to `manage_url`, and that starting again is a new Checkout session (ADR 2026-09-09). | Diagram + example + manage section present. |
...
| Id | Requirement | Acceptance |
| --- | --- | --- |
| FR-DOC-040 | SDKs page has tabs TypeScript · cURL for each of the ten frozen methods (SDK FR-SDK-007, including `subscriptions.list`); the TS tab is the §4.2 snippet. Python is hidden entirely until `elapse` is published (BR-DOC-001). **Amended 2026-09-17 (signed, FR-DOC-047).** | Tab set snapshot; the ten methods match the SDK export list. |
...
| FR-DOC-042 | Every SDK method in the docs maps to one operation in the committed OpenAPI file and vice versa; `docs-site/ci/surface-check.ts` diffs the SDK's exported method list, the SDKs page's method list, and the file's `operationId`s. **Amended 2026-09-17 (signed, FR-DOC-047).** | Check passes; adding a doc for a non-SDK method fails it. |
...
| FR-DOC-046 | **Payouts** page (`payouts.mdx`, nav after Subscriptions): settlement cadence (hourly, at once on stop/cap) and the two destinations, the fee as a share of settled seconds with the rate deferred to the dashboard, the payout address rules, the ledger and CSV, and the general AUSD → bank path (exchange or issuer redemption) without naming unverified providers. Target of the dashboard's Cash out sheet (FR-DSH-121). | Page exists in nav; the dashboard link resolves (no 404). |
| FR-DOC-047 | **React SDK in the docs** ([ADR 2026-09-17](../decisions/2026-09-17-react-sdk-replaces-hosted-checkout.md)). A "React" page documents `<ElapseProvider>`, `<Authorize>`, `<Meter>`, `<Receipt>`, `<TxLink>`, the hooks, events, theme variables and sound, each with a snippet synced from `examples/saas` (FR-DOC-011). The Quickstart's checkout step renders `<Authorize>` instead of opening a URL. The surface check (FR-DOC-042) also diffs `@elapse/react`'s exports against the React page. | Surface check fails on an undocumented export; Quickstart CI still green. |
...
| 2026-09-09 | Claude (for William) | FR-DOC-022 gains the "Let subscribers manage their meter" section on `manage_url` ([ADR 2026-09-09 manage_url](../decisions/2026-09-09-subscription-manage-url.md)). **Awaiting William's signature.** |
| 2026-09-09 | William | Signed the FR-DOC-022 manage section amendment. |
...
| 2026-09-23 | Claude (for William) | **FR-DOC-047's CDN build answers the question it was written for.** William, reading the write-up: "so a dev cannot use another tool for frontend and just use the ts sdk alone?" They can — the framework-free `mount()` build has been documented since 2026-09-19 — but it sat under **No bundler?** at the foot of a page called React, which a Vue, Rails or Django developer never opens, and nothing on the TypeScript page said the frontend was free. The section is now **Without React**, says in its first line that Vue, Svelte, Rails, Django and plain HTML are all fine, and explains that the build carries its own React so none is installed; the TypeScript page opens with a Note pointing at it. No nav change, so signed FR-DOC-002's ten entries stand. One test in `pages.test.ts`; 24 docs tests, validate and broken-links green. |
| 2026-09-23 | Claude (for William) | **FR-DOC-002 amended, and FR-DOC-047's CDN build gets its own page — signed 2026-09-23 (William).** Pointing at a section was not enough: "why not have separate page for no react people or a way to easily get there". The SDKs group now holds three pages — TypeScript, React and **Without React** (`sdks/browser`) — and the ten top-level entries do not move, since SDKs is a group. The new page carries the whole non-React path: the server is unchanged and the Quickstart's cURL tabs cover it, the page fetches its `cs_` from the merchant's own endpoint, the `mount` options in full, entitlement decided on the webhook rather than on a browser callback that a closed laptop never fires, and Pause/Resume as requests to the merchant. The React page keeps a three-line pointer; the TypeScript page's Note aims at the page. `/without-react` and `/sdks/without-react` redirect to it. 26 docs tests, validate and broken-links green. |
| 2026-09-23 | Claude (for William) | **FR-DOC-021's snippets now compile where they are pasted.** Checked the landing page's integration snippet against the published `@elapse/sdk@0.3.1` by installing it and running `tsc --strict`: every method, parameter and type was real, and one line was not. `req.headers["x-elapse-signature"]` is `string | string[] | undefined` in Node, `constructEvent` takes `string | undefined`, so the first webhook handler a merchant copies fails to compile — in the docs as well as on the landing page. Both working examples already narrow it (`Array.isArray(h) ? h[0] : h`) and neither page showed that. Signatures now narrows in the TypeScript and Express snippets and says why, and notes that Hono's `header()` needs no narrowing. All three compile under `--strict` against the published package. The landing page names the header instead, since a hero snippet is read rather than pasted, and gains its first test. |

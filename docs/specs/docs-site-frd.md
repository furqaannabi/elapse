...
| FR-DOC-042 | Every SDK method in the docs maps to one operation in the committed OpenAPI file and vice versa; `docs-site/ci/surface-check.ts` diffs the SDK's exported method list, the SDKs page's method list, and the file's `operationId`s. **Amended 2026-09-17 (awaiting sign-off, FR-DOC-047).** | Check passes; adding a doc for a non-SDK method fails it. |

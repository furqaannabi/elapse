...
| FR-RCT-042 | **The docked meter** (signed 2026-09-19). `<Meter dock="bottom-right" | "bottom-left">` renders the meter as one fixed capsule instead of a card: a dot that says running or paused, the elapsed clock, and the amount, with the accent on the amount while it runs. Controls the product allows are a second capsule beneath it; a merchant-started meter shows none (FR-CHK-037). The held state reads "$X held" and the end state stays in the corner as the receipt. Omitting `dock` renders inline exactly as before, so no merchant's layout moves. On a phone the capsule spans the gutter. Nothing loops: the dot marks a transition once (BR-CHK-002). | Component tests per corner and state; the inline default renders no `.elapse-dock`; the dot's `data-running` follows the server. |

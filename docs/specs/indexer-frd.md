...
| BR-IDX-008 | **No ordering across logs.** Envio runs Effects concurrently, so the ingest POSTs for the logs of one block (even one transaction) reach the platform in any order. The platform derives cumulative state from the log that carries it (`StreamCanceled` totals, `Deposited.totalDeposited`) and never from arrival order (API FR-API-071). Observed live 2026-09-05: `StreamCanceled` landed before its `Settled`. |
...
| 2026-09-05 | Claude (for William) | BR-IDX-008 added after the live cancel run: same-block logs arrive out of order; the platform must be order-independent. Subscriber cancel proven live (tx `0x8837e73f…2b2528`): `invoice.settled` + `subscription.canceled` verified 29 s after the start click. |
